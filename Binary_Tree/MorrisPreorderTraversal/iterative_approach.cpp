...
void preorderTraversal(TreeNode* root, vector<int> & preorderArr){
    ...
    // traverse the tree
    while(curr){
        // if the left child is NULL, add the value to the preorderArr and move to the right child
        if(curr->left == NULL){
            preorderArr.push_back(curr->val);
            curr = curr->right;
        }
        // if the left child is not NULL, find the predecessor
            ...
            // if the predecessor is not NULL, add the value to the preorderArr and move to the right child
            else{
                prev->right = NULL;
                curr = curr->right;
            }
