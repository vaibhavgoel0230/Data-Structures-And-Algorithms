...
vector<int> leftView(TreeNode* root){
    // result vector to store the left view
    vector<int> results = {};
    ...
    while(!q.empty()){
        ...
        for(int i=0; i<levelSize; i++ ){
            ...
            // if it is the first node at the current level, add it to the result vector
            if(i==0) results.push_back(node->val);
...
int main(){
    ...
    vector<int> results = leftView(root);
