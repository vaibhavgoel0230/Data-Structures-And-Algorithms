...
int ceilInBST(TreeNode* root, int target){
    ...
    if(!root) return -1;
    // if the root is equal to the target, return the root data
    if(root->data == target) return root->data;
    // if the root is less than the target, then the ceil is in the right subtree
    if(root->data < target){
        return ceilInBST(root->right, target);
    }
    // if the root is greater than the target, then the ceil is in the left subtree
    int ceil = ceilInBST(root->left, target);
    // if the ceil is greater than or equal to the target, return the ceil, otherwise return the root data  
